...
--
-- The order is forced by two dependencies, each found by `verify:rollbacks` executing this file,
-- not by reading it: `knowledge_admin_version()` RETURNS the version table's row type, so it must
-- go before the table; the chunks read policy CALLS `knowledge_version_readable()`, so that must
-- go after it.
