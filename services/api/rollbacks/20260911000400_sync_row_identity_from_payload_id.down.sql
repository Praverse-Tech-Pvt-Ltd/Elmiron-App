-- Rollback for MR-24 B -- sync row identity taken from the payload's own id.
--
-- Restores `apply_sync_item` to its state immediately before this migration, which is the
-- body installed by `20260911000200_apply_sync_item_nested_coordinates.sql`: nested
-- coordinates, and row identity taken from `p_entity_id`.
--
-- THIS REINSTATES THE WORST DEFECT THIS PROJECT HAS FOUND. `push-client.ts` sends
-- `entityId: body.visitId` as a queue-screen GROUPING key; the restored body uses it as each
-- record's PRIMARY KEY. `capture_consent` opens with
-- `select ... where c.id = p_id; if found then return v_existing`, so a doctor's SECOND
-- answer on a visit finds the first and returns it -- accepted, unchanged, silent. A
-- withdrawal of consent is discarded and the MR is told it was recorded.
--
-- Every clinical row for a visit also collapses onto the visit's id again: one check-in, one
-- check-out, one consent record, one samples row and one call report per visit, forever.
