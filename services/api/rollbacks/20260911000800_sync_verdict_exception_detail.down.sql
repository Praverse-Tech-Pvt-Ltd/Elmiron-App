-- Rollback for BE-W97 -- the refusal FIGURES on the sync verdict.
--
-- This restores `sync_push` byte-for-byte as 20260908000600_sync_verdict_sqlstate.sql left it,
-- which is the version this change was made against.
--
-- WHAT APPLYING THIS MEANS. `sqlDetail` and `sqlHint` stop being emitted, so every
-- `raise ... using detail` in this schema goes back to dying inside `sync_push`'s exception
-- handler. Concretely: a 45004 reaches the MR as a sentence with no cap, no month-to-date and
-- no period -- the state MR-27 C2 measured and refused to call a pass. 45001, 45007 and 45008
-- lose their figures with it.
--
-- THE CLIENT MUST BE ROLLED BACK WITH IT, and this is the half that fails loudly rather than
-- quietly. `SyncPushResultSchema` requires both keys, so a client on the new contract talking
-- to this function fails to PARSE the push response -- every write is reported as a transport
-- failure and re-queued rather than silently losing its numbers. Loud is the better failure
-- and it is deliberate, but it means this is not a server-only rollback.
