-- Rollback for MR-44 B3 -- `beat_plan_entry` leaves the pull again.
--
-- WHAT APPLYING THIS MEANS. The beat-plan screen goes back to having no stops to render:
-- `beat_plan_entries` holds rows, nothing carries them to the handset, and `buildDayRoute`
-- maps an empty `entries` to an empty route. That is the `BE-W89` state, and MR-14's answer
-- to it was to leave the screen on the mock rather than present an empty result as fact.
--
-- The client half must go with it. `SyncPullEntitySchema` in `packages/core` and the
-- exhaustive switches in `apps/field/src/sync/pull.ts` all name the entity, and
-- `sync-pull-contract.spec.ts` pins the entity set exactly -- so a database rolled back
-- without the client rolled back fails the build, loudly, which is the intended direction.
--
-- `updated_at` is dropped LAST, after nothing reads it. The column was the whole re-size:
-- `sync_pull` orders and pages on `(updated_at, id)` and the table had no such column.
--
-- Note it cannot be un-dropped with its values: the timestamps are gone. A re-applied
-- forward migration gives every row the new `now()`, which means one more full re-sync.
...
alter table public.beat_plan_entries drop column if exists updated_at;
