-- Rollback for MR-12 Part D1/D2.
--
-- Restores record_check_out and apply_sync_item to the definitions captured with
-- pg_get_functiondef immediately before the change, drops the constraint pair and the
-- column.
--
-- **This is destructive of product data, and says so.** Any visit recorded as `not_met`
-- has no other status to become: `completed` would claim the doctor was seen and
-- `cancelled` would claim the MR never went. Rolling back therefore rewrites them to
-- `cancelled` -- the less flattering of the two lies, chosen deliberately, because the
-- alternative inflates an MR's completed count with visits that did not happen and does
-- it silently.
...
declare
  v_count bigint;
begin
  update public.visits set status = 'cancelled', not_met_reason = null
   where status = 'not_met';
  get diagnostics v_count = row_count;
  if v_count > 0 then
    raise notice
      'rollback rewrote % not_met visit(s) to cancelled and destroyed their reasons. '
      'Those MRs attended those visits.', v_count;
  end if;
end $$;
...
grant execute on function public.record_check_out(
  uuid, uuid, double precision, double precision, timestamptz, double precision,
  public.capture_source) to authenticated;

-- The enum member itself is NOT removed. PostgreSQL cannot drop an enum label, and
-- recreating the type would require rewriting every column that uses it. A label nothing
-- writes is inert; the constraint pair and the column are what gave it meaning.
