-- FIX-06 Part C — dedicated SQLSTATEs for the two shift-window refusals.
--
-- The error contract in `packages/core` maps a SQLSTATE to something an MR can act on.
-- That only works where the code identifies the refusal. `22023` does not: it is raised
-- **64 times** across these migrations for everything from a missing consent version to a
-- malformed argument. Mapping it to "your shift window is wrong" would be a guess, and
-- matching on the message text instead would make English the contract -- so copy could
-- not be changed without breaking a client.
--
-- Two refusals need to reach the MR as different actions:
--
--   45002  no shift window is configured for this territory (or the org default expired).
--          Nothing the MR can do; their manager must configure working hours.
--   45003  the check-in is outside the configured window. The MR can act: it is not a
--          fault, they are early or late, and the app should say which.
--
-- `45001` (FIX-02, consent notice superseded) established the range. These are the same
-- shape: project-defined codes for refusals with a remedy, deliberately outside the four
-- standard codes the schema otherwise uses.
--
-- The three function bodies below are otherwise **unchanged** -- they are the live
-- definitions with the errcode substituted, so this migration alters no behaviour beyond
-- the code that identifies the refusal.

-- 45002 — no window configured, or the organisation default has expired.
...
-- 45003 — outside the window, in record_check_in.
...
-- 45003 — outside the window, in record_check_out.
