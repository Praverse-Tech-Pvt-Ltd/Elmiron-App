-- MR-26 B1 -- the consent NOTICE joins the pull, so consent can be captured offline.
--
-- MR-25 D1 measured that three of the five writes cannot be performed with no signal. Consent
-- was one: `notices.ts` calls `active_consent_text` live, so the screen rendered "Could not
-- load this visit. The app could not reach the server." and the doctor was never asked.
--
-- MR-25 filed that as a product trade-off between working offline and never displaying
-- unconfirmed state. That framing was wrong. "Never display what the server has not confirmed"
-- is a rule about ASSERTING FACTS; it is not a rule about gating actions on a live round trip.
-- The pull exists so the client can hold server-confirmed state and act on it offline. A
-- consent text version is server-issued, tenant-scoped, low-volume and immutable except for
-- retirement -- it is exactly what a pull is for.
--
-- THREE THINGS THIS DOES NOT CHANGE, each named because each is a rule someone could think
-- this bends:
--
--   1. MR-12 Q4 stands. `consent_record` stays in `c_omitted`: ~3,000 audit rows a day, for
--      reinstall-only value. A text VERSION carries neither cost.
--   2. FIX-02 stands. The client still sends a version id the SERVER issued, and
--      `capture_consent` still re-resolves the active notice at `captured_at` and refuses
--      45001 when they disagree. The client is not choosing which notice is in force; it is
--      carrying the server's answer in its pocket.
--   3. Tenancy stays the database's. The new arm has NO predicate, because `sync_pull` is
--      SECURITY INVOKER and the RESTRICTIVE policy from BE-W79 scopes it -- the same reason
--      the doctor and clinic_address arms carry none.
--
-- The `updated_at` column this arm orders on is added by the migration beside this one.
