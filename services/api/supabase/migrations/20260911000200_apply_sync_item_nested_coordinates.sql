-- MR-24 B -- `apply_sync_item` read check-in coordinates FLAT; the contract nests them.
--
-- The second defect underneath the first. With the shift-window wrap fixed, the next
-- check-in from the emulator came back:
--
--   rejected / missing_reference
--   null value in column "latitude" of relation "check_ins" violates not-null constraint
--
-- The body the app actually sends -- built by `checkInRequest()` against
-- `CreateCheckInRequestSchema` in `packages/core` -- is:
--
--   { id, visitId, source, occurredAt,
--     coordinates: { latitude, longitude, accuracyMetres, capturedAt } }
--
-- `coordinates` is a NESTED object and has been since the schema was written. It is what the
-- REST endpoint takes and what the mock at :4010 accepts. This function read
-- `p_payload ->> 'latitude'` at the top level, which is always null for a real client body,
-- so `record_check_in` was handed nulls and the NOT NULL constraint refused the row.
--
-- **Check-in and check-out have therefore never worked end to end from a real client.**
--
-- WHY NO TEST CAUGHT IT, which is the part worth keeping: `gate1.spec.ts` -- the suite whose
-- header says it simulates "one MR's day entirely through the sync path, the same path a
-- real device uses" -- builds its items by hand, FLAT, to match this function. Five payload
-- literals, none produced by `checkInRequest()`. The test and the function agreed with each
-- other and neither agreed with the product.
--
-- The contract is right and the database is wrong, so the database moves. Deliberately NO
-- fallback to the flat shape: a `coalesce(p_payload->'coordinates'->>'latitude',
-- p_payload->>'latitude')` would let both shapes live forever and would let Gate 1 keep
-- passing against a body no client sends. The tests are corrected in the same change.
--
-- Only the three coordinate reads move. The body below was produced from
-- `pg_get_functiondef` and edited in exactly those places, so nothing else can have drifted.
