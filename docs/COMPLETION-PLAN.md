...
| **BE-W92** | **A DDL test deadlocks against concurrent DML across vitest workers** | `services/api/tests/tenant-boundary-restrictive.spec.ts`, `services/api/vitest.config.ts` | MR-22 A2 | — | 2 | Run `34472586019`, database job: `error: deadlock detected` (SQLSTATE `40P01`), 605 of 606 passed. `D2 — a restrictive tenant boundary cannot be widened > and with the RESTRICTIVE policy removed, the same policy widens immediately` does `drop policy … on public.<table>`, which needs an `AccessExclusiveLock`, while another worker holds a `RowExclusiveLock` from an insert: *"Process 2200 waits for AccessExclusiveLock on relation 16458; blocked by process 2202. Process 2202 waits for RowExclusiveLock on relation 17262; blocked by process 2200."* Classic ABBA across two relations. <br><br>**It passed on a straight re-run, and a re-run is not a fix** — it is timing-dependent, so a green re-run says only that the interleaving did not happen that time. Likely made more probable by MR-18's `dimension-coverage.spec.ts`, which adds another worker doing writes inside `inRolledBackTransaction`, though that is a hypothesis and NOT measured — the previous flake in this repo was diagnosed twice from plausible reasoning and wrong both times. <br><br>**Prior art exists and is the place to start:** `services/api/tests/auth.ts` serialises identity creation across workers with `pg_advisory_lock(IDENTITY_LOCK_KEY)`, the mechanism that replaced `maxWorkers: 6` in MR-12 C. An advisory lock only helps if BOTH sides take it, so the DML side has to participate — which is why this was registered rather than patched at the end of a session. Verification: run the api suite repeatedly under load and record the failure rate BEFORE changing anything, the same discipline BE-W85 asks for |
