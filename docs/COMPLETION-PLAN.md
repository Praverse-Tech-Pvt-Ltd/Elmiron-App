...
| ID | Title | Changes | Deps | Blocker | Est | Verification |
|---|---|---|---|---|---|---|
| **BE-W83** | Restrictive tenant policies for the `visible_user_ids()` tables | ~30 policies, `visits`, `check_ins`, `consent_records`, `call_reports`, `samples_and_inputs`, `recordings`, `voice_notes`, `beat_plans`, `analyses`, the `sync_*` and `visit_audio_quarantine*` tables | MR-07 D | **a measurement, not a decision** | 3 | MR-07 D made the tenant boundary restrictive on the seven tables that carry a tenant or reach one in a single hop. These thirty carry no tenant column, so a restrictive policy must reach one through `mr_id -> user_profiles.organisation_id` or `doctor_id -> doctors.organisation_id` — a correlated subquery per row, **on top of** the `visible_user_ids()` subquery the permissive policy already runs. Whether the planner collapses the two or doubles them decides whether this is free or a pilot-scale regression, and this repo has already been bitten once by a predicate that looked free and disabled an index (the LEAKPROOF finding). **Verification — CORRECTED MR-52 E1; the original could not measure this.** It named `team_activity` and `coverage`, which are `SECURITY DEFINER` owned by `postgres` (`BYPASSRLS`): **no policy runs inside them**, so the before/after would have been identical whatever the policies did, and a green result would have meant nothing. Measure the paths the policies are actually on: (1) `sync_pull`, which is `SECURITY INVOKER` and is the phone's hot path; (2) a direct table read per role — `select count(*) from visits` as an MR, a manager and an admin — which is what PostgREST does; both with `explain (analyze, buffers)` against `seed:synthetic --mrs 100 --history 1y` (3,520 doctors, 208,800 visits), before and after, **plus the row counts each role sees, which must be identical** or the policy has removed legitimate rows rather than cost. Then the same D2 proof pair — an over-broad permissive policy must fail to widen, and must widen the moment the restrictive one is dropped. **Where else those two functions are named:** `BE-W107` and its status row, and the MR-47 notes on the day rule, all of which are about WHICH DAY a visit counts on and are unaffected — nothing else in this register measures a boundary through them. The general rule, worth carrying: a `SECURITY DEFINER` function owned by `postgres` can never demonstrate a row-level policy, in either direction |
...
| **`BE-W30`** — status | **DONE (MR-50 B), and not by a placeholder.** `TranscriptV1` is the real contract (`packages/core/src/field/transcript-v1.ts`): source, stable segment ids and order, required-nullable segment confidence, per-token language for code-switching, and code-point span offsets that redaction and citations point into. `transcript-v0.expiry.test.ts` now validates a realistic Hinglish fixture instead of checking a date — the ship-or-cut question it forced was answered on 22 September (`C7`) |
| **`BE-W32`** — status, the next AI milestone | **Run the vendor bake-off — BLOCKED, honestly: it needs 5–10 hours of LABELLED MR–doctor audio, and the project has none.** `C7` changes its output: "cut the AI layer" is no longer a valid result; the result is a vendor. Real consultations cannot supply the audio before the §8.6 signatory (`C3`, `C8`). **Proposed unblock** — a corpus the team records itself, staged role-play between consenting employees in Hinglish: `blocked-on-you` → *MR-50, the bake-off corpus*. Nothing has been recorded. `BE-W31` (the harness) does not need the audio and can proceed |
| **`BE-W104`** — status | **CLOSED (MR-50 C1).** `delegation.spec.ts` now asserts `issue_recording_upload_grant` inherits `begin_upload`'s own-visit check: the visit's MR gets a grant (positive control); another MR of the same organisation and an MR of the rival organisation are refused (42501). Mutants: the wrapper inlined as a direct insert with no ownership check fails both refusals; inlined with a COMPANY check instead of ownership fails only the same-organisation case. That leaves `BE-W105`'s four as MR-45 recorded them: three tested (`approve_call_reports_bulk`, `active_consent_text`, now this), `is_admin` with no boundary to cross |
| **`BE-W100`** — status | **CLOSED (MR-50 C2).** `list_analysis_overrides` builds camelCase keys explicitly (`20260922000100`); `analysis-overrides-contract.spec.ts` parses the REAL response through `ListAnalysisOverridesResponseSchema`. Against the rollback both cases fail; a key renamed to a different camelCase name fails only the parse case. The stale "`BE-W95`" citation in `endpoints.ts` is corrected |
| **`BE-W110`** | **`create_analysis_override` returns the raw table row — snake_case — and the client parses it as `AnalysisOverrideSchema` (camelCase).** Same defect as `BE-W100`, on the write: `rls.spec.ts` reads `analysis_id` off its result. **And the client does not reach it at all:** `client.createAnalysisOverride` POSTs to `/analyses/:id/overrides`, a path only the mock serves — PostgREST has no such route. So the real write path needs both: the RPC returning the contract's shape, and the client calling the RPC. Found in MR-50 C2 (a first draft of this row said the client "would throw on a real response"; checked, it would not reach one). Not fixed in C2, to keep it to the read the brief named. **Needed by `FE-W12`** (MR-50 F) if it writes overrides |
| **`BE-W88`** — status | **CLOSED (MR-50 C3).** `GeofenceCentreSchema` (latitude, longitude) for a clinic's centre; `Coordinates` keeps provenance for captured fixes. `fromClinicAddressRow` carries the centre when both halves exist. The mock's fixture had invented an accuracy and a capture time for a centre — removed. Verified on a REAL pull payload (the centre round-trips intact) and the negative half (a check-in without provenance is still refused). Mutant: the mapper back to `null` fails only the round-trip case |
| **`BE-W83`** — measurement (MR-50 C4) | **Measured; policies NOT built — ROOM.** From the catalogue: **10 tables are function-scoped** (RLS forced, no grant to `authenticated`, no policy): `analyses`, `analysis_overrides`, `audio_destruction_log`, `audio_purge_runs`, `audit_log`, `consent_records`, `restore_reconciliation_findings`, `restore_reconciliation_runs`, `transcripts_raw`, `transcripts_redacted`. **None has a restrictive tenant policy, and none can usefully have one:** all 85 `SECURITY DEFINER` functions are owned by `postgres`, which holds `BYPASSRLS`, so a policy on these tables is never evaluated for the functions that read them — the function bodies are the whole boundary (as `C1`/MR-42 already relied on). **19 tables are directly readable with permissive policies only** — `BE-W83`'s real scope: `adverse_event_reports`, `app_thresholds`, `beat_plan_entries`, `beat_plans`, `call_report_approvals`, `call_reports`, `check_ins`, `check_outs`, `recordings`, `samples_and_inputs`, `sync_batches`, `sync_events`, `sync_item_reinstatements`, `sync_items`, `upload_grants`, `visit_audio_quarantine`, `visit_audio_quarantine_clearances`, `visits`, `voice_notes`. **7 carry a restrictive tenant boundary:** `clinic_addresses`, `consent_text_versions`, `doctors`, `organisations`, `territories`, `territory_shift_windows`, `user_profiles`. Building the 19 needs the register's own verification — `explain (analyze, buffers)` against the one-year synthetic seed — which did not fit beside D–G |
| **`FE-W54`** — status | **CLOSED (MR-50 D4), on the emulator.** The voice-note screen reads the visit from the pulled store; "Save this note" from a real visit (`0809df1c`, *"Your note · Dr Vikram Rao (DEMO)"*) moved the recording to `files/voice-notes/<rep>/<note>.m4a` and said *"Saved on this phone, in your notes. It has not been sent"*. The mock is no longer called |
| **`FE-W53`** — status | **CLOSED (MR-50 D2), on the emulator**, under `C9` (kept): "Start again", recording over, and leaving without saving delete the recorder's file; leftovers from a force-stop are swept when the screen opens. Kept notes live in the signed-in rep's own folder and no other rep's session lists or opens them (`voice-note-files.ts`) |
| **The voice-note UPLOAD — what it needs (MR-50 D5, recorded, not built)** | **Server, already built** (from the catalogue): `begin_upload(visit, 'voice_note', size, seconds)` issues an `upload_grants` row with a server-made `storage_key` (visit must be the caller's; 15-minute grant); the private `audio` bucket (50 MB object limit) whose INSERT / UPDATE / SELECT policies all require `has_live_upload_grant(name)`; `record_upload_progress(grant, bytes)` and `resume_upload(grant)` for the resumable path; `complete_upload(grant, object_id, seconds, size, recorded_at, bitrate)` finalises the `voice_notes` row, taking the size Storage observed; `abandon_upload(grant, reason)`; `my_upload_queue()`; `close_stale_upload_sessions()` is the server's own sweep (no signed-in grant). **Client, missing:** (1) methods for those RPCs — `packages/core`'s `UploadSession` describes a REST shape (`uploadUrl`, `uploadedBytes`) that no server route returns, so the contract and the RPCs must be reconciled first; (2) the byte upload to `audio/<storage_key>` with the MR's token; (3) **`FE-W29`** — recordings and voice notes cannot go through `sync_push` without an `uploadGrantId`, so an offline note needs its own queued upload, per rep (MR-49's rule); (4) delete the kept file only after `complete_upload` confirms; (5) **`FE-W46`** — send the real size, now readable from the file (`expo-file-system`'s `File.size`), instead of the literal `1`. **Open question before building:** whether Supabase Storage's resumable (TUS) endpoint can be driven without a further client library — not checked; a new library would be a dependency to ask for |
| **`FE-W63`** — status | **CLOSED (MR-50 E1), on the emulator.** The banner's title is the write the MR PRESSED (`blockedWrite`, fixed at the press), not the stage read at render. Offline check-in on rep B's visit `36e65ca7`: *"Checked in — waiting to send"* with *"This check-in cannot be sent yet"* — it had said check-out. Flushed on reconnect; the server holds one check-in by rep B. Route test with the stage visibly moved; mutant (the route not passing the pressed write) kills only it |
| **`FE-W64`** — status | **CLOSED (MR-50 E2), on the emulator.** Once the pull has settled without the visit, the screen says *"This visit is not on this phone"* and offers no check-in; while loading it claims nothing. As rep B, rep A's `0809df1c` by direct link: *"Getting this visit"*, then the sentence. Mutant (never says absent) kills only the settled case |
| **`FE-W12`** — status | **CLOSED (MR-50 F), verified by a real render.** `C10`'s renderer, dev-only: `@testing-library/react` **16.3.3** with its required peer `@testing-library/dom` **10.4.1** and `jsdom` **20.0.3** as the DOM environment — the last two already in the tree (via `jest-environment-jsdom` and `vitest`), declared at the same versions rather than added anew; all pinned exactly; no non-test file imports them. The console's `vitest` runs `.test.tsx` in jsdom by a per-file opt-in; everything else stays in node. Added `client.listAnalysisOverrides` (POST `/rpc/list_analysis_overrides` — the RPC Supabase serves, camelCase since MR-50 C2) and the same route in the mock, checked by the mock's contract spec. The review screen shows *"Overrides already logged (n)"* above the form, *"could not be loaded"* on a failed read (never "none"), *"No override logged yet"* only on an empty answer. Deleting the fetch fails the saved-override and none-logged cases; not rendering the rows fails only the saved-override case. **Not driven in a browser**: the console has no running harness here |


### Added by MR-51 — the review record and the upload

| Item | Detail |
|---|---|
| **`BE-W110`** — status | **CLOSED (MR-51 B1–B3), against local Supabase, no mock.** `create_analysis_override` now returns the contract's camelCase `jsonb` (`20260922000200`, dropped and recreated because a return type cannot change in place; grants restored and asserted equal to the catalogue's before). `client.createAnalysisOverride` POSTs to `/rpc/create_analysis_override`; the mock serves that path instead of `/analyses/:id/overrides`. `analysis-overrides-http.spec.ts` drives the console's own client over real HTTP with a GoTrue password sign-in: the owning admin saves, the row carries that admin and the organisation of both the reviewer and the analysis, and the history lists it; the rival organisation's admin is refused save and read by the scope check (`42501`, with a reason given, so it is not the reason rule refusing) and writes nothing. Mutants: the rollback live fails the save and the owner control; the scope check deleted fails only the rival save |
| **`FE-W12`** — REOPENED and fixed (MR-51 B2) | **MR-50 closed it against the mock, which never asks for a reason. Against the real server every admin saw "could not be loaded"**: `list_analysis_overrides` refuses an admin read without one (`22023`). `loadOverrides` now sends `OVERRIDES_READ_REASON` on every read, whoever reads — the admin screen's `READ_REASON` pattern — so the console does not decide which roles need one. Proved with the panel rendered from the REAL read, mock running and then killed (port 4010 closed): saved rows `3e66f02f…` and `b4b40a46…`, reviewer the fixture admin, audit rows `178389` and `178391` carrying the reason. Mutant: the reason dropped fails only the render case |
| **`FE-W66`** | **The console cannot reach the real server as a signed-in user — it has no sign-in at all.** Every page builds its client with `getAccessToken: () => Promise.resolve(null)`, so against Supabase every RPC answers `28000 not authenticated`. And the review page loads its analysis from `GET /analyses/:id`, a path only the mock serves (the database has `read_analysis`); `listAnalyses` likewise. So `BE-W110`'s fix makes the WRITE path real, and it is proved through the console's own client and panel — **but the running console in a browser still cannot show it.** Needs: a console sign-in (a design question plus, likely, a dependency such as `@supabase/ssr` — to be ASKED), the analysis reads moved to their RPCs, and PostgREST's error body (`{code, message}`) mapped — today the client reports a scope refusal as `internal_error`, HTTP 403. Found MR-51 B2 |
| **`BE-W83`** — status | **BUILT for 18 of the 19 directly readable tables (MR-51 C3); the 19th is `BE-W106`.** **C1 probe first** (`tenant-probe-direct-tables.spec.ts`, real HTTP, GoTrue sign-in): one row of the rival organisation in each table, read by an MR and an admin of the first; the rival's own user reads it as the positive control. **18 do not leak** — `visible_user_ids()` filters by organisation inside its body (MR-06), so every permissive policy was tenant-bounded by that one function. **`app_thresholds` leaks** (`using (true)`) — `BE-W106`; left open by the operator (`C13`) and asserted as-is so a fix flips the test. **C3:** `20260922000300` adds a restrictive `*_tenant_boundary` to each of the 18 through the existing helper — `mr_id in (select p.id from user_profiles p where p.organisation_id = current_user_organisation_id())`, one hop further through `visits`, `beat_plans`, `call_reports` or `sync_items` where the row has no `mr_id`; `sync_events` by former MR or former territory. The migration's guard asserts exactly these 18, restrictive, to `authenticated`, through the helper in both `using` and `with check`. **The register's own verification was wrong and was not used as written:** it named `team_activity` and `coverage`, which are `SECURITY DEFINER` owned by `postgres` (BYPASSRLS), so no policy runs inside them. Measured instead against `seed:synthetic --mrs 100 --history 1y` (208,819 visits), median of 5, before → after: `sync_pull` as an MR (SECURITY INVOKER, the phone's hot path) **562 → 560 ms**; `count(*) visits` as MR 13.5 → 19.4 ms, manager 25.3 → 34.7 ms, admin 33.9 → 51.9 ms; `check_ins` MR 14.3 → 16.2, manager 25.8 → 26.6. Rows seen identical before and after (MR 2,088, manager 83,520, admin 208,800) — nothing legitimate lost. The plan evaluates the organisation subquery once (`hashed SubPlan`, `loops=1`) and keeps the index-only scan: one extra hash probe per row. `beat_plan_entries` and `call_report_approvals` timings are vacuous — the seed makes none. **Proof:** a mirror pair for the `mr_id` shape (an over-broad permissive policy cannot widen; drop the restrictive one and it does) and the catalogue test extended from 7 tables to 25. **C4:** `visits_tenant_boundary` dropped live → exactly one test fails (the catalogue test); the C1 probe stays green, which is the point of a second layer |
| **`FE-W29`** — status | **BUILT (MR-51 D), emulator-verified end to end against local Supabase.** A kept voice note is an ORDINARY sync item: `voiceNoteQueueItem` → `sendOrQueue` → the outbox's own retry, per-rep queue and dead-lettering. `uploadVoiceNote` (in `push-client.ts`) checks the note is the signed-in rep's, asks `begin_upload` for a grant with the file's REAL size (`FE-W46` closed — `expo-file-system` 57.0.2's `File.size`), writes the bytes to the key the server issued, finalises through `sync_push` (`apply_sync_item` → `complete_upload`), and only then deletes the phone's copy. A note the server already holds is not uploaded twice. **Emulator evidence:** online save → `voice_notes` `826579f2` 5 s / **65,231 bytes** = exactly the file on the phone, `upload_status uploaded`, `purge_after` stamped by `stamp_audio_retention` (2026-12-21), object in the private `audio` bucket at 65,231 bytes `audio/mp4`, grant `completed`; offline save (cold-started with the reverse removed) → *"Saved on this phone. It will send by itself when you have signal, and is then removed from this phone"*, queue shows `voice_note` waiting, nothing on the server; on reconnect it uploaded once (`10b56052`, 6 s, 71,961 bytes) and the phone's copy went. **Exactly once:** flushed again — one row, one `sync_items` row per note, two objects, two completed grants. **D5, two reps on one phone:** rep A's kept unsent note stayed untouched while rep B was signed in and online for 25 s (3 rows, not 4), and rep B's own note uploaded under B's `mr_id` into B's own folder. Mutants: the already-stored check removed fails only the lost-acknowledgement test; the visit sent as the sync entity instead of the note fails only the item-shape test |
| **`BE-W111`** | **The server names every voice-note object `.opus`; the phone records AAC in an MP4 container (`.m4a`).** `begin_upload` builds `'voice-notes/…/….opus'` and a CHECK on `voice_notes.storage_key` requires that suffix; `RecordingPresets.HIGH_QUALITY` on Android writes AAC. MR-51 D uploads with `contentType: 'audio/mp4'`, so Storage records the true type (`metadata.mimetype = audio/mp4` on the uploaded object) and only the KEY lies. Harmless to playback today because the type is stored beside it; it will mislead anything that reads the extension — a transcription vendor handed the key, or an operator listing the bucket. **Fix is a server decision, not a client one:** either the key carries the real container, or the recorder produces Opus. Found MR-51 D3 |
| **`FE-W67`** | **Offline past the access token's life, the rep is signed OUT and loses their day and their queue screen.** Measured on the emulator: a cold start with no server reachable and a token minted the day before landed on "Sign in"; the same cold start with a fresh token restored the session and said *"Your day as of 10:08 — not confirmed since"*. So the session survives being offline, but NOT a token that has expired while offline — and `FE-W19`'s gate is a full offline DAY, longer than an hour. The queued work is not lost (it is keyed per rep and waits for that rep to sign in again), but the rep cannot see it or add to it. Needs a decision on offline token lifetime or a refresh-free offline mode. Found MR-51 D4 |
| **Voice notes kept BEFORE MR-51** — recorded | Two notes from MR-50 are still on the Pixel 10 (`b95aa032…`, `15bdcba4…`), from reps whose accounts no longer exist after a `db reset`. They have no queue row, so nothing will ever send them: the upload is queued at SAVE time and MR-50's saves predate it. Not a defect to fix on the emulator, but a real one for any device that recorded a note on an older build — a sweep that enqueues unsent kept notes would close it |

### Added by MR-52 — the console

| Item | Detail |
|---|---|
| **`FE-W66`** — status | **CLOSED (MR-52 A), proved in a browser against local Supabase.** **A1, sign-in:** `@supabase/ssr` **0.12.7** with `@supabase/supabase-js` **2.117.0**, both pinned exactly, in `apps/console` only (`apps/field` stays on its locked 2.112.3 — the lockfile gained entries and moved nothing). It is the SAME identity as the app (`D-14`): one `auth.users` row, one `user_profiles` role, no second user table. Cookies rather than `localStorage`, because the pages are Server Components and the server must read the same session the browser wrote. `middleware.ts` refreshes the token on every request and redirects a signed-out browser — **the redirect is a courtesy, not the access control**, which A5 measures separately. **A2:** one `signedIn()` builds the client for every page, with PostgREST as the base URL and the request's own token; `20260923000100` makes `read_analysis`, `list_analyses` and `list_consent_records` emit the contract's camelCase through one shared row shaper; consent notices are read from the table the tenant boundary already scopes, mapped by `fromConsentTextVersionRow` (MR-23 B1). A test reads every console source file and fails if any of them builds a client with a null token or names `:4010` again |
| **A2 — what each console read reached, before and after** | `listAnalyses` REST `/analyses` (mock-only) → **`/rpc/list_analyses`**; `getAnalysis` REST `/analyses/:id` (mock-only) → **`/rpc/read_analysis`**; `listConsentRecords` REST `/consent-records` (mock-only) → **`/rpc/list_consent_records`**; `listConsentTextVersions` REST `/consent-text-versions` (mock-only) → **the `consent_text_versions` table through PostgREST**; `getMe` REST `/me` (mock-only) → **dropped**, the signed-in email replaces the name it supplied; `listAuditLog` and `getRetentionStatus` were already RPCs and are unchanged; `listAnalysisOverrides` and `createAnalysisOverride` became real in MR-50/51. The mock now serves the three new RPC paths too, checked by its contract spec |
| **Two columns the contract declared and the schema does not have** | `AnalysisSchema.transcriptId` is now **nullable** and `findings` comes back **empty**, because `20260811000100` records that findings and transcript citations "arrive with the analysis engine in week 10" and transcripts do not exist until week 8. The console therefore renders *"Nothing was flagged"* on a real analysis, which is true, and the coaching queue lists nothing, because the queue lists finding categories that RECURRED and there are no findings yet. Inventing either would be `BE-W56` in a new place |
| **A3/A4/A5 — browser evidence** | Driven with Playwright (installed in a scratch directory with `--no-save`, **not** a repo dependency; the browser was already on the machine). **A3:** signed in as an organisation's admin, the review screen showed the real analysis (`Analysis d579344d · rubric v1.0 · MR-52 demo seeded-by-hand`), the history said *"No override logged yet"*, the form saved an override, and after a reload the page showed *"Overrides already logged (1)"* with the exact reason. Row `68e9c81f` in Postgres carries that admin and that organisation. **Elimination:** the mock was killed (`curl` → `000`, nothing listening on 4010) and the same pages still rendered the analysis and the history. **A4:** the other organisation's admin got *"This analysis could not be loaded"* and no override text; the owning admin, in the same run, saw both. **A5:** every page (`/`, `/coaching`, `/admin`, `/coaching/:id`) redirected an unauthenticated browser to `/sign-in` and leaked no id — and, separately, all four RPCs answered **401 `42501` permission denied** to the publishable key with no user token, which is what actually refuses |
| **`FE-W67`** — status | **FIXED (MR-52 B), measured on the emulator before and after.** **B1, the current behaviour and its boundary:** the cut-off is the ACCESS TOKEN's expiry, not the refresh token's. `supabase-js` keeps the session in storage when a refresh fails for a network reason — it treats that as retryable — but `getSession()` still answers `{ session: null }` once the access token has expired, and `session.tsx` read that null as signed out. Measured with `jwt_expiry` lowered to 120s locally (production is 3600s, restored after): signed in 11:26:30, a cold start at 11:28:29 — one second before expiry — stayed on Today; a cold start at 11:31:01, offline and past expiry, landed on **Sign in**. At the real 3600s that is the first cold start after about an hour offline. **What the rep loses:** every screen behind the sign-in — Today, the visit, check-in and check-out, and the queue screen. The queued work itself is safe on disk under their own key, and unreachable until they sign in, which needs signal. **B2:** `persisted-session.ts` asks storage when the library answers null, and the app treats a session that is still stored as signed in with a stale token. **This decides nothing about permission** — an expired token is refused by the server exactly as before — and the "offline" versus "rejected" distinction is the library's own: a non-retryable refusal REMOVES the entry. The client's storage key is now named (`fieldforce.auth.v1`) so the app can read what the library wrote; devices holding a session under the old derived key sign in once more. **B4:** with the token five minutes expired and no signal, the rep opened a visit and checked in — *"Checked in — waiting to send"*, *"This check-in cannot be sent yet"* (`FE-W63`'s wording intact), nothing on the server — and on reconnect it landed exactly once (`35cc6ff1`, `geofence_status inside`). **B3:** with signal and the refresh token revoked server-side, the next cold start after expiry signed the rep out, as it must; `FE-W56`'s `PGRST303` wording is on the pull-failure path and is untouched |
| **`BE-W111`** — status | **FIXED (MR-52 C1).** `begin_upload` mints `.m4a`, the container `expo-audio` actually writes; both `storage_key` CHECKs accept `.opus` OR `.m4a`, so the three objects MR-51 uploaded stay valid rows; `recordings.codec` accepts `aac` and now DEFAULTS to it, because `complete_upload` inserts without naming a codec and the old default would have made a future recording claim Opus. `AnalysisSchema`'s neighbour `RecordingSchema.codec` widens from `literal('opus')` to `enum(['opus','aac'])`. **What else read that extension, checked rather than assumed:** the two CHECKs, the codec CHECK and default, `packages/core`'s recording contract, four API suites that build their own keys (they construct, they do not assert the server's), two that DID assert the minted name (`upload.spec`, `consent-audio.spec` — both updated), and the mock's two fixtures. Storage policies key on `has_live_upload_grant(name)` and read no extension. **Owed when a second container exists:** the format belongs in `begin_upload`'s arguments; one producer writing one container does not justify that signature change today |
| **Voice notes kept before the upload** — decided (MR-52 C2) | **They are DELETED, and the rep is told.** Not a preference: a kept file is named by its note id, and the VISIT lives in the queue row, so a note saved before MR-51 D has no visit — and `begin_upload` takes one. There is no upload path to give them, so "upload them later" was never available. `removeUnsendableNotes` removes any kept note the rep's own outbox is not holding, when the voice-note screen opens, and the screen says *"N older note(s) could not be sent — they were saved before sending existed, so the visit was not recorded with them. They have been removed from this phone."* Emulator: a planted note in the signed-in rep's folder was removed and the sentence shown; the queued note beside it was untouched. **What this deliberately does NOT do:** touch another rep's folder. Two notes on the Pixel 10 belong to accounts that no longer exist; the ownership rule (`C9`, MR-49) says a session opens only its own rep's folder, and deleting another rep's audio from a shared phone is what that rule exists to prevent. They go when that rep signs in, or when the device is wiped |
| **`BE-W106`** — status | **The LEAK is closed; the MODEL question is untouched and now DATED (MR-52 D).** `20260923000300` revokes SELECT on `app_thresholds` from `authenticated` and `anon` and drops the `using (true)` policy, so the table is no longer readable directly by anyone signed in — which removes the cross-tenant read MR-51 C1 measured, and with it the exposure of another company's territory id and `set_by_user_id`. **D2, nothing broke, established not assumed:** exactly one database object reads the table (`threshold(text, uuid)`, `SECURITY DEFINER` owned by `postgres`, so grants to `authenticated` are irrelevant to it), and no client reads it at all — not the app, not the console, not the mock. Every value reaches the app through a function already. **D1's "scoped function" half was NOT built, deliberately:** nothing needs one today, and an unused read path is the speculative code this repo refuses. **D3, two-sided:** an MR and an admin are refused over real HTTP (403, `42501`, *"permission denied for table app_thresholds"*), the rival admin cannot reach the other company's `set_by_user_id`, and the positive controls still pass — `threshold()` answers and `my_shift_window()` resolves. Restoring the grant fails exactly those refusals. **Still open, and unchanged:** `threshold()` still returns any territory's value to any caller who names it, and a `global` row is still shared by every tenant. That is the model question, and it is `blocked-on-you` 2.7 |
| **`BE-W106`** — D4, the acceptance now expires | **Proposed date: `2026-10-31`, for the operator to confirm.** Chosen because it is when the repository's other recorded acceptance expires — the production migration-drift acceptance (`--accept-undeployed-until 2026-10-31`) — so two accepted gaps come due in one conversation rather than drifting apart. The mechanism is the one `BE-W21` already established: a dated row in `app_thresholds` (`be_w106_settings_model_decision_due`), a status function (`be_w106_decision_status()`), and `check:decision-debt` failing CI once it passes — "a non-zero exit from a CI job reaches a person, and a null in a table does not". **Resolution is read from the SCHEMA, not a flag:** the status reports resolved when `app_thresholds` carries organisation scoping — an `organisation_id` column or a `scope` value for it. It fails closed: a missing deadline reads as overdue, so the alarm cannot be silenced by removing the row that carries it. Moving the date is a dated, attributable row, because the table is append-only |

### Added by MR-53 — the recording path

| Item | Detail |
|---|---|
| **A1 — what stands between "the doctor agreed" and "a recording row exists"** | **Built and tested, server side:** `capture_consent(...)` records the answer against the exact notice version, with `validate_consent_capture` bounding the device clock; `active_consent_text`/`active_consent_text_at` decide which notice is current for a tenant and language; `begin_upload(visit,'recording',size,seconds)` refuses unless a STANDING consent exists (consented, not a withdrawal, not superseded by one), refuses a quarantined visit, enforces the 25 MB / 2 h ceilings and the per-MR storage ceiling, and mints the `upload_grants` row with a server-made `storage_key` (15-minute slide, 24-hour hard expiry); `issue_recording_upload_grant(visit,size,seconds)` is the thin wrapper (`BE-W104`); the private `audio` bucket's INSERT/UPDATE/SELECT policies all require `has_live_upload_grant(name)`; `record_upload_progress`, `resume_upload`, `abandon_upload` and `my_upload_queue` carry the resumable path; `assert_upload_still_permitted(grant)` re-checks consent FIRST, then state, then expiry; `complete_upload(...)` finalises, takes the size **Storage observed**, and inserts the `recordings` row whose `require_consent_for_recording` trigger re-checks the consent row and `stamp_audio_retention` sets `purge_after`; `cascade_consent_withdrawal` revokes the grant and removes downstream audio when a withdrawal lands; `close_stale_upload_sessions` sweeps. **Missing, client side:** the record control is unreachable (below), there is no client call to any grant function for a recording, and nothing writes a transcript at all |
| **A2 — called by the app, versus never called by the app** | **Called by the app today:** `begin_upload` (MR-51 D, voice notes only), `complete_upload` (through `sync_push` → `apply_sync_item`, voice notes only), `capture_consent` (through `sync_push`), `active_consent_text` (consent screen), plus `sync_pull`, `sync_push`, `my_shift_window`, `daily_mileage`. **NEVER called from the app, though tested in the database:** `issue_recording_upload_grant`, `record_upload_progress`, `resume_upload`, `abandon_upload`, `my_upload_queue` (named only in a comment), `read_consent_record`, `clear_audio_quarantine`, and the `recording` branch of `apply_sync_item`. **That is the unexercised-code class, and its cause is one line:** `app/visit/[id].tsx` holds `const consents: readonly ConsentRecord[] = []` because **`sync_pull` does not carry `consent_record`** — a declared phase-2 omission (MR-21 B6), not an oversight, and `sync_pull`'s own `completeness.omittedEntities` says so. With no consent record on the phone, `recordingBlock([])` returns `never_asked` and no control is drawn |
| **MR-53 B — the record control, behind two off switches** | **`recording_permission(visit)`** answers from the SAME standing-consent predicate `begin_upload` enforces (`standing_consent_for_visit`), and `recording-permission.spec.ts` drives both and asserts they agree in each direction — a read that said yes where the write says no would show the MR a control that then fails. Reasons are distinguished because the sentences differ: `never_asked`, `declined`, `withdrawn` (new — "the doctor said no" is false of somebody who agreed first), `quarantined`, `feature_off` and `not_your_visit` (both silences). **The flag is ENFORCED, not documented:** the server half is an `app_thresholds` row shipped **false**, and the client half REFUSES TO LOAD — `loadAppConfig` throws and names the host — if a build asks for recording while pointing anywhere but a local stack; an unreadable config reads as off. A build that may not record does not even ask the server. **`C3` is not reversed:** scope §8.6 still requires the named PV/DPDP signatory |
| **MR-53 B4 — a withdrawal mid-recording** | The existing cascade wins, and now has a caller that could be mid-recording when it fires: the open grant goes to `revoked`, the next byte is refused **for the consent reason** (`42501`, *"consent has been withdrawn"*) rather than for the grant's state — the order inside `assert_upload_still_permitted` exists precisely so the MR is not told their recording was malformed when the doctor changed their mind — and the screen's own read flips to `withdrawn`. The client keeps the file rather than destroying it on a refusal: a refusal is not proof the audio should not exist, and the MR is told |
| **MR-53 C — the recording uploads through the voice-note path** | One sender (`uploadAudio`), not two. The only differences are the kind asked of `begin_upload` — where the consent check lives — and the folder: a recording is kept in the rep's `recordings/` folder, **not `cache/Audio`, because the voice-note screen sweeps that cache and would delete it**. It replays as an ordinary sync item with the outbox's retry, per-rep ownership and dead-lettering; the server issues the key and the client uses it as given (`BE-W111`'s `.m4a` holds); `complete_upload` still takes the size Storage observed. The screen no longer POSTs to the mock. Mutants: the folder forced to `voice-notes` fails five recording cases, the kind forced to `voice_note` fails exactly one, and the voice-note suite stays green in both |
| **MR-53 D — `TranscriptV1` can be written** | `ingest_transcript(jsonb)` writes into **`transcripts_raw`**, which has existed since MR-14 (with `transcripts_redacted` beneath it) — which is why the withdrawal cascade already deleted transcripts. **Callable by `service_role` only:** `authenticated` and `anon` have no EXECUTE, so an MR and another organisation's admin are refused by the grant before a row is read. It refuses: an unknown `schemaVersion`; a transcript longer than its audio; a segment ending past the end; segments that overlap; a transcript filed against a different visit than the audio's; a destroyed recording; and **a visit whose consent is no longer standing** — without that last one, a job in flight would write words back after the destruction log said the audio was gone. Idempotent on the transcript id. **No vendor client was built (D4):** there is no vendor and no corpus, which is `BE-W32` and waits on the operator. D5 mutant: the consent check removed fails exactly one test |
| **MR-53 E1 — how long a rep survives offline, and where the boundary actually is** | **The two tokens have different lives and only one of them is a clock.** The ACCESS token is `jwt_expiry = 3600` (`services/api/supabase/config.toml:189`) — one hour — and that is the boundary MR-52 B1 measured on the emulator, with `jwt_expiry` temporarily at 120s so the far side could be reached in an afternoon. The REFRESH token has no expiry in this repository's configuration at all: rotation is on (`:195`) with a 10-second reuse interval (`:198`), and **`[auth.sessions]` is commented out** (`:296-300`) — no `timebox`, no `inactivity_timeout`. So nothing here ends a refresh token by the passage of time; it is ended by being used (rotation) or by being revoked. **The weekend, therefore:** a rep offline from Friday 18:00 to Monday 09:00 (~63 hours) loses the access token at 19:00 on Friday and from that moment is signed in only because `persisted-session.ts` says so — every screen reads the pulled store, every write queues under their own key, and on Monday's first signal `autoRefreshToken` exchanges the refresh token and the outbox flushes. Sixty-three hours is no worse than two. **What would break it is not the clock, and that is the point:** (1) an operator setting `[auth.sessions].timebox` or `inactivity_timeout` on the hosted project — the commented examples are **24h** and **8h**, and either one signs out every rep across a weekend, with `persisted-session.ts` correctly NOT saving them, because a session the server has ended is a non-retryable refusal and auth-js removes the stored entry; (2) the same refresh token rotated somewhere else more than 10 seconds earlier, which is reuse detection. **Marked honestly: this is a CODE AND CONFIGURATION answer, not a measurement.** `config.toml` configures the LOCAL stack only — it says so at `:180-182` — so production's `jwt_expiry` and session timeouts are whatever the dashboard says, and this repository cannot state them. **That is an operator question, and it is the one that decides the weekend.** Nobody has left a device offline for 63 hours either; the hour was measured, the weekend is reasoned |
| **`FE-W68` NEW — the phone and the console are on two different `supabase-js` versions, and only one of them is pinned** | `apps/console` pins `@supabase/supabase-js` **2.117.0** and `@supabase/ssr` **0.12.7**, both exactly. `apps/field` declares **`^2.112.3`** — a caret — and the lockfile resolves **2.112.3** (`pnpm-lock.yaml:52`, `:114-116`, `:8292-8302`). Both trees are installed side by side, with their own `@supabase/auth-js`. **The reason, which is a good one:** `@supabase/ssr@0.12.7` declares a peer of `@supabase/supabase-js: ^2.114.0` (`pnpm-lock.yaml:2173`), so the console could not stay where the phone is when MR-52 A added it; and moving the PHONE's auth library inside a console change would have shipped an untested change to the thing reps depend on. MR-52 A's stated aim was that the lockfile 'gained entries and moved nothing', and it held. **Why it is worth an id anyway.** `persisted-session.ts` is written against auth-js BEHAVIOUR — that a retryable refresh failure leaves the stored entry alone and a non-retryable one removes it — and that behaviour was measured on **2.112.3** (MR-52 B1–B4). The console does not use `persisted-session.ts`, so the divergence costs nothing today. It becomes a risk the moment the field is bumped, because the bump silently re-dates that measurement while nothing in CI re-runs it. **And the caret is the sharper half:** the console's version can only move when somebody chooses it, the phone's can move on any `pnpm install` that re-resolves, without anybody choosing anything. **Convergence condition, so this is recorded rather than accidental:** `apps/field` moves to the console's exact version and is pinned exactly — no caret — in a change OF ITS OWN, whose evidence is the MR-52 B4 offline case re-run on a device: token expired, no signal, check in, reconnect, lands exactly once. Not folded into a change about something else. Registered in MR-53 E2, not fixed there, for the reason in the row |
| **MR-53 E3 — two dated decisions in one CI step: one warning or two?** | **Two — and until this change the second one arrived carrying the first one's question.** `check:decision-debt` builds `warnings` by concatenating both debts' arrays, so each debt that is inside its window contributes one line. The dates overlap by design: `BE-W106` is due **2026-10-31** and enters its 21-day window on **2026-10-10**; the UCPMP cap is due **2026-11-06** and enters its window on **2026-10-16**. **So for fifteen days both warn, and from 2026-10-31 one is RED while the other is still amber.** The defect that state exposed: the CLI printed the UCPMP paragraph — 'What is the UCPMP sample cap, on what dimension, and who at the client owns that number?' — after **every** warning and **every** failure, because MR-52 D4 added the second debt to the step and left the advice belonging to the loop. A reader would have been sent to ask the client about a sample cap in order to clear a question about which settings belong to which company, and on 2026-10-31 that wrong paragraph would have arrived attached to a red build with `enforce_ucpmp_sample_cap() is inert` under it. That is this repository's own named failure mode — advice that looks like a control while pointing somewhere else. **Fixed:** the question and the consequence belong to the debt (`DEBTS`), `evaluateAllDecisionDebt` is pure and exported, and the CLI prints one block per unanswered decision with its own label, question and consequence. Six tests, no database and no calendar: both warning is two warnings and still green; each warning carries its own question and the settings one names neither `UCPMP` nor `enforce_ucpmp_sample_cap`; `BE-W106` overdue fails **alone** while the cap is still amber; answering one does not clear the other; both answered is silent; either status unreadable fails closed. **Mutant:** giving the settings debt the UCPMP question kills exactly one test and leaves the other ten green. The database half of `decision-debt.spec.ts` is SKIPPED locally (the stack is stopped) and runs in CI's *migrations · Gate 0 RLS suite · rollbacks* job |
| **MR-53 E3 (the question as it was asked) — the duplicated deadline row** | **I answered a different question first, and this is the one that was asked.** MR-52 recorded, as a mistake of its own, that the LOCAL database carries **two identical `be_w106_settings_model_decision_due` rows** — the migration was re-applied by hand after a rollback drill, and `app_thresholds` is append-only so neither can be removed. A database built from the migrations has exactly one. **Does `check:decision-debt` then say it twice? No — one warning.** `threshold()` reads `order by (territory_id is not null) desc, effective_from desc limit 1` (`20260815000100_thresholds_and_shift_defaults.sql:63-76`), so one row is read, one status is built, one warning is produced; and `app_thresholds_unique_version` forces the two rows to differ in `effective_from`, so which one wins is deterministic rather than arbitrary. The duplicate is **invisible**, not merely harmless. **The sharper half, pinned in the same test:** 'the latest row wins' is only safe while the newest row is in EFFECT. `effective_from <= now()` excludes a future-dated row, so a deferral written with a future `effective_from` would look filed and change nothing until that date — the old deadline keeps governing and CI still goes red on it. **Marked honestly: both tests are DB tests and are SKIPPED here** — the stack is stopped at the operator's instruction — so this is reasoned from the SQL and proved by CI's *migrations · Gate 0 RLS suite · rollbacks* job, not measured on this machine |

### Added by MR-54 — the consented case, proved and broken

| Item | Detail |
|---|---|
| **`FE-W69` NEW — the visit screen asks the server once and never asks again** | `apps/field/app/visit/[id].tsx:115-130` runs `recordingAvailability` in a `useEffect` keyed on `[visit?.id]`. It fires on mount and on nothing else — no focus listener, no interval, no invalidation. **A screen left in the navigation stack therefore keeps whatever permission it fetched when it mounted, for as long as the app lives.** Found on the emulator in MR-54 A2: a screen instance mounted at 17:18 (before the doctor was asked) still showed *"This phone does not have the doctor's answer for this visit"* at 17:33, while the server — asked over the app's own PostgREST path with the app's own bearer token — answered `{"reason":"allowed","allowed":true}`. A cold start rendered it correctly, which is what makes it a staleness bug rather than a disagreement about the rule. **The harmless direction is the one I hit; the harmful one is the reverse:** a doctor withdraws, the rep navigates back to a stale screen that still offers **Record this visit**, and the microphone runs on a doctor who has said no. Nothing unlawful is STORED — `begin_upload` refuses — but the audio exists on the phone, and `FE-W70` is why it stays there. **Not fixed in MR-54:** the fix is a policy choice (re-ask on focus, on an interval, or on a server signal) and it changes how often every visit screen calls the server |
| **`FE-W70` NEW — a withdrawal cannot stop a recording that is already running** | **Measured on the emulator (MR-54 A5), and this is the session's stop.** With a recording in progress, a withdrawal was written to `consent_records` and `standing_consent_for_visit` went to **null** while the phone was still capturing. **The recording did not stop.** It ran until the rep pressed *Stop recording*, and **518,740 bytes of audio of a doctor who had withdrawn were written to the phone** — then kept there, because MR-53 B4 deliberately keeps the file on a refusal. **What DID work, and it is the whole server boundary:** `begin_upload` refused, no grant was ever minted (still 4 grants, all `completed`, none issued after the withdrawal), nothing reached Storage, and the rep was told in the server's own words — *"The recording was not accepted: visit 11ae8015… has no standing consent; there is no upload path"*. **Why the phone cannot do better today, stated rather than guessed:** there is no channel by which a withdrawal reaches a recording device. The phone holds no consent ledger by design (MR-21 B6), it asks the server once per mount (`FE-W69`), and **the client cannot even capture a withdrawal** — `is_withdrawal` and `supersedes_consent_record_id` are never sent, which is `BE-W95`, open since MR-28. So the withdrawal in this test had to be written server-side, and in production it could only arrive from somewhere the phone never reads. **This needs an operator decision, not an engineering guess:** whether a withdrawal must destroy audio already captured on the device, which contradicts MR-53 B4's recorded rule that a refused recording is KEPT (`recording-upload.test.ts:149-152`, *"a refusal is not proof the recording should be destroyed"*). Both cannot hold. MR-54's brief asserted the destroy side (A5, *"nothing is left on the phone"*); the repository has the keep side under test. **Refused to silently pick one** — see `blocked-on-you` MR-54 |
| **`BE-W112` NEW — the recording is not audited; the consent that authorised it is** | Measured from the live catalogue, not read from a migration: **16 tables carry `write_audit_row`** — including `consent_records`, `visits`, `analyses`, `adverse_event_reports` and `app_thresholds` — and **`recordings`, `voice_notes` and `upload_grants` carry none.** `recordings` has exactly two triggers, `recordings_stamp_retention` and `recordings_require_consent`. So after MR-54 A2's four successful uploads, `select … from audit_log where table_name in ('recordings','upload_grants')` returns **zero rows**. The asymmetry is the point: the system records that a doctor agreed and does not record that audio of them was created, which is the artefact the agreement was about. `ingest_transcript` (MR-53 D) writes its own audit row by hand, so the intent exists for transcripts. **Not fixed in MR-54** — an audit trigger on an upload path is a write-path change and belongs with the `BE-W83` policy work, not bolted onto a proving session |
| **MR-54 A2/A3/A4 — what the emulator proved** | **A2 (emulator).** Consent `consented` captured at 11:49:44Z; the control **Record this visit** appeared — the first time it has ever been reachable on a device; *"Recording · agreed at 17:19"* took its time from the server's consent row, not the device clock. Recorded 30s by the on-screen timer, stored `duration_seconds` **31**. Object present at `recordings/e0fce4ad…/2d864d2c….m4a`; `recordings.size_bytes` **382527** equals `storage.objects.metadata->>'size'` **382527**; mimetype Storage observed is **`audio/mp4`** and the key ends **`.m4a`** — `BE-W111` holds, extension and container agree. `purge_after` **2026-12-22T11:51:01.715Z** is exactly `received_at` + 90 days and is stamped by `recordings_stamp_retention`, a trigger; the client's payload carries no retention field at all. Phone's copy **gone** after the server confirmed (`files/recordings/<rep>/` empty). **Honest limit:** equality of the two byte counts does NOT by itself prove the server ignored the client's claim, because the client claimed the same number. The discriminating proof is `upload.spec.ts:898`, which finalises with a deliberately wrong `1` and asserts the stored value is what Storage received — that is **code**, not emulator. **A3 (emulator).** Offline is established two-sided with a tool that exists on the image: `nc` to 127.0.0.1:54321 answers open with the `adb reverse` in place, **connection refused** without it, open again when restored. *(An earlier `curl` check was VACUOUS — curl is not on the image at all, so `CURL_FAILED` proved nothing. Recorded because it is exactly the "a command that exits 0 is not a command that worked" trap, inverted.)* Recorded offline → *"Recording saved on this phone. It will send by itself when you have signal"*, file on disk, server unchanged. Reconnected → uploaded, and the recording id **is** the filename that was on the phone (`37908b24…`). Flushed again → still **2 recordings, 2 keys, 2 grants, 2 objects**, phone folder empty. A third offline recording repeated it on a cold-start flush. **A4 (emulator), two-sided.** Sign-out says *"Nobody else who signs in on this phone will see or send them"*. Signed in as a second rep on the same phone: Today said **Everything sent**, their settings showed **no** unsent banner, no second `recordings/` folder was created, the first rep's stranded `3b5352a4….m4a` (108,943 bytes) was **untouched**, and the server count did not move. **Positive control:** the owner signed back in and it sent at once — folder empty, count 3 → 4 |

### Added by MR-54 (continued) — B1, B2/B3, C1/C2

| Item | Detail |
|---|---|
| **MR-54 B1 — the commit check is a mechanism now** | `.githooks/pre-commit`, wired with `core.hooksPath` (`pnpm hooks:install`), running **typecheck, lint and format** and refusing the commit on any failure. **No new dependency** — a shell hook and `scripts/ci-local.mjs`. **It does not list the steps itself.** `ci-local.mjs` already derives them by reading `ci.yml`, for a reason that file argues at length: a hand-kept parallel list has drifted three times in this repository, always in the direction of claiming MORE coverage than it has. So the hook names three steps by their COMMAND and a new `--only=` flag finds them — and **every pattern must match exactly one step**, so a `ci.yml` rename makes the hook FAIL rather than silently check less. Controls, all three exercised: a pattern matching none throws; `--only=pnpm` (11 matches) throws; `--only=` with no patterns throws. **Positive control (the refusal):** a staged `const x: number = 'string'` produced `src/shared/b1-control.ts(2,14): error TS2322` → *COMMIT REFUSED* → **`git log` unchanged at `e7664e1`**. **Positive control (the pass):** this session's own commits went through it. **Stated limits, not hidden:** it checks the WORKING TREE not the index (the husky/lint-staged trade); it does not run the tests, because a hook costing minutes is a hook people `--no-verify` past, and a bypassed control is not a control; and it cannot stop `--no-verify` at all — git allows the bypass by design, so CI remains the enforcement and this is the thing that tells you before the push. **A fresh clone must run `pnpm hooks:install`** — git does not read `core.hooksPath` from the repository, which is a security property, not an oversight |
| **MR-54 B2/B3 — the three suites that could not skip, and both counts** | `consent-notice-tenancy` and `dimension-coverage` had a whole `describe` without `.skipIf(!reachable)`; `organisation-backfill` had **one test** inside a describe whose other two read migration files and are MEANT to run with the stack down, so that one got `it.skipIf` and the describe was left alone — a describe-level skip there would have quietly stopped checking the repository for `min(id)`, which is the check that found that defect twice. **Both counts, as asked:** with the stack **STOPPED**, `pnpm test` now exits **0** — api **14 files passed / 46 skipped**, **101 passed / 712 skipped**, where before it was **3 files / 9 tests FAILED**. With the stack **RUNNING**, the api suite is **60 files / 813 tests, all passed, none skipped** — so the nine do not merely skip, they run and pass. **The control that matters: the 101 passing with the stack stopped is unchanged**, so nothing that used to run was skipped to buy the green. **Any other suite with the same shape?** The empirical answer is no — a stack-stopped full run is now clean, which is a stronger check than grepping for `describe(` without `skipIf`, because a describe with no guard is perfectly correct when its tests touch no database (`decision-debt.spec.ts` has one deliberately) |
| **`BE-W113` — FOUND AND CLOSED in the same change: `pnpm test` could report a cached stack-stopped run as a green one** | Found while doing B3 rather than by looking for it. With the stack **up**, `pnpm test` reported api as **101 passed / 712 skipped** — the stack-STOPPED numbers. `turbo`'s cache key for `@fieldforce/api:test` is file content; **whether Postgres is listening on 54322 is not a file**, so a green from a stopped stack is replayed over a running one. `npx turbo run test --filter @fieldforce/api --force` gave **813 passed** on the same tree, which is the proof. **This is the failure shape this repository keeps naming** — something that looks like a control while covering nothing — and B2 makes it WORSE, because before B2 a stopped stack at least failed loudly. **Closed:** `services/api/turbo.json` sets `cache: false` on that task only. CI runs the suite once, so it loses nothing. Verified: the task now reports `8 cached, 9 total` with the api one executing |
| **`BE-W114` NEW — an intermittent api failure under parallel load, NOT fixed** | **Observed twice in four `pnpm test` runs today and never in three standalone `vitest run` runs of the same tree.** Once it named the test: `retention-ops.spec.ts > the watchdog against the real database > backlog-based stall detection > does not trip on a single object briefly overdue -- the false-positive case`. The second occurrence was counted but its name was not captured, and **it is not recorded here as the same test, because that was not observed**. The named one is time-sensitive by construction — *briefly* overdue — which is a plausible mechanism under the load of seven workspaces' suites running together, and plausible is not measured. **Not chased:** finding it properly means repeated runs under controlled load, which is its own session. **Why it is registered rather than shrugged at:** a suite that fails one run in two under the command developers actually type teaches people to re-run until green, and that habit costs a real failure later |
| **MR-54 C1/C2 — `FE-W68`'s risky half closed** | `apps/field` now pins `@supabase/supabase-js` at **`2.112.3`** exactly; the caret is gone. **The lockfile diff is one line** — the specifier — and the resolved version is unchanged, which is the whole point: this removes a silent change vector, it does not upgrade anything. `pnpm install --frozen-lockfile` still passes, so CI is unaffected. The console stays at `2.117.0`. **C2 — what re-testing convergence would cost, so the decision arrives with a price.** Moving the field to 2.117.0 is not a version bump, it is invalidating a measurement: `persisted-session.ts` is written against auth-js BEHAVIOUR — a retryable refresh failure leaves the stored entry alone, a non-retryable one removes it — and that was established on 2.112.3 by MR-52 B1–B4 **on a device**, because no test in this repository exercises it. So convergence costs: (1) an emulator run of MR-52 B1 (cold start past token expiry, offline, must stay signed in), (2) MR-52 B3 (refresh token revoked server-side, must sign OUT), (3) MR-52 B4 (offline check-in queued, lands exactly once on reconnect), and (4) MR-54 A2/A3 re-run, because the upload path goes through the same client. **Roughly a session**, and it cannot be shortened by reading the auth-js changelog, because what is being re-tested is this app's behaviour at the boundary, not the library's stated behaviour |

### Added by MR-54 (continued) — the two backend items closed

| Item | Detail |
|---|---|
| **`BE-W112` — CLOSED. The recording is audited now, and the grant's progress noise is not** | `20260924000100_audio_rows_are_audited.sql`. `recordings` and `voice_notes` are audited on insert, update and delete; `upload_grants` on insert and delete, and on update **only when `state` changes**. The filter is not tidiness: `record_upload_progress` updates a grant on EVERY CHUNK — `bytes_received`, `chunk_count`, `last_progress_at`, a sliding `expires_at` — and never touches `state`, so an unfiltered trigger would write one row per chunk and bury `issued -> revoked` under progress. A trail nobody can read is the same failure as no trail, reached from the other side. **What this does NOT claim, pinned by its own test:** destruction was never the gap. `audio_destruction_log` has recorded what was destroyed, when and why since `20260815000300`. The gap was CREATION and the custody change before it. **Checked before writing, not after:** `audit_log.actor_id` and `actor_role` are nullable and `current_app_role()` reads the JWT through `nullif`, so a trigger on `recordings` cannot make the purge worker fail on a row it is entitled to change — the actor is simply null, which is true: a job is not a person. **Guards:** the migration asserts `write_audit_row` exists, the three tables exist, `upload_grants.state` exists, and **that none of the three was already audited** — if one were, the measurement this is built on had changed and the migration stops. Afterwards it asserts four triggers exist AND that the grant's update trigger carries a `when` clause, because the flooding version would otherwise pass the count. **Mutants, both directions:** removing the `when` clause kills exactly one test (the progress half); dropping the triggers kills three, including the state-change half of the same two-sided test. Suite: **61 files / 818 tests**, `verify:rollbacks` clean, no drift (73 of 73) |
| **`BE-W114` — CLOSED, and my first diagnosis was WRONG** | I registered it as *"an intermittent api failure under parallel load"*. **It is not about load at all.** `audio_purge_is_stalled()` reads the WHOLE of `recordings` and `voice_notes` and trips on either a backlog over `purge_batch_limit x purge_backlog_multiplier` or an oldest-overdue age past `purge_max_silence_hours` (12h). Both are global, and the false-positive test asserts that global function returns **false** after inserting one 4h-overdue row of its own. **So a single committed row more than twelve hours overdue — from any earlier session, a synthetic seed, an abandoned probe — fails it on a tree where nothing is wrong.** **Measured, not reasoned:** one committed recording backdated 13 hours flipped `audio_purge_is_stalled()` false → true and failed that test on demand; deleting it passed again. **And the reason it LOOKED intermittent is the better half of the finding:** the retention suite commits real purges, so the first run on a poisoned database fails and *destroys the backlog in the process*, and the next run is green. Self-clearing state is indistinguishable from flakiness from the outside, which is why the first diagnosis reached for load. **Fix:** `onlyOurOverdueRows(client)` pushes every pre-existing overdue row out of the window INSIDE the rolled-back transaction, before either stall test inserts anything, so both assert their own data on any machine. Applied to the backlog test too — on an already-stalled database that one passed **vacuously**, which is the same defect wearing a green tick. **Two-sided control on the same poisoned database:** helper removed → the test fails; helper restored → it passes. **One thing learned by accident and worth keeping:** while poisoned, **23 other tests failed** — `begin_upload`, the storage policy, the resumable-session suite. A stalled purge is a fleet-wide interlock on new uploads, which is a designed behaviour this register had not stated anywhere |

### Added by MR-54 (continued) — `BE-W96`

| Item | Detail |
|---|---|
| **`BE-W96` — CLOSED. `recorded_at` is bounded in both directions** | `20260924000200_recorded_at_is_bounded.sql`. `complete_upload.p_recorded_at` was the device's word with no bounds while `capture_consent` had bounded `captured_at` in both directions since FIX-12. It now refuses a future claim (**45009**) and one older than the server will accept on the device's word (**45010**), plus an explicit null. **Why it is more than a wrong-looking date:** `recorded_at` is what a coaching queue orders by and what a later citation of a consultation points at, so a recording that claims to predate its own consent is a real conversation filed under a time that never happened. **New codes, not 45007/45008, and this is the load-bearing decision.** The client turns a SQLSTATE straight into a sentence: 45007 reads *"your consent was captured in the future"*. Reusing it would tell a rep their CONSENT was refused when their RECORDING was — the one-sentence-for-four-codes defect `refusalForSqlState` exists to end. **And the contract is enforced, not remembered:** `error-contract.spec.ts` derives every raised code live from `pg_proc.prosrc` and fails if one is missing from `BY_SQLSTATE`, so minting these without wiring them reddens CI. Both are in the contract as `recording_in_future` and `recording_too_old_to_accept`, `actionable: true`. **That claim dragged the change into the client, deliberately:** `actionable: true` with no remedy would be a claim the code does not honour, so `explanation.ts` gains two sentences — the 45009 one says the recording itself is fine, the 45010 one says waiting will not change it. **Thresholds reused, with the caveat stated:** `consent_future_tolerance_seconds` (120s) and `consent_max_sync_lag_hours` (72h) bound the same physical thing — one phone's clock — and two unverified numbers for one question is how the forgotten one drifts. The keys are NAMED for consent; if audio must differ that is a new dated row. **Mutants, one per bound:** disabling the forward check kills exactly the 45009 test; disabling the backward check kills exactly the 45010 test; the other five stay green each time, which is what shows the positive controls are not passing because everything is refused. **One existing test collided and was corrected rather than deleted:** `upload.spec.ts`'s retention-clock test passed a **200-day-old** `recorded_at` to prove `purge_after` is server-stamped, and 200 days is now refused. Changed to **71 hours** — the oldest claim the server still accepts — which keeps it discriminating, because a `purge_after` derived from `recorded_at` would land near now + 87 days and still fail its assertion. **Not addressed, and older than this change:** a refusal at finalise leaves the bytes already in Storage and the grant open until it expires, exactly as the existing size and duration refusals do. Suite **62 files / 824 tests**; field 40 / **613**; rollbacks clean; drift 74 of 74 |

### Added by MR-54 (continued) — `BE-W102`

| Item | Detail |
|---|---|
| **`BE-W102` — CLOSED for in-body refusals, and the 2026 costing had a fifth route** | MR-40 B costed four escape routes and chose **doing nothing**, written down so an assessor was told. The operator reopened it, so this reverses a recorded decision deliberately. **The route that costing did not have:** PostgreSQL still has no autonomous transaction — that part was right — but *a request does not have to fail as an ERROR to fail as a 403*. PostgREST lets a function set `response.status` and RETURN, so the transaction **commits** and the audit row survives while the caller is still refused. No dependency, no `dblink`, trail stays in the database. Measured before anything was written, and again after. **In-database callers still raise**, because `current_setting('request.method', true)` is `POST` over PostgREST and null in psql — so no existing test and no internal caller changed, which is what makes the blast radius small. **THREE functions, not seven — the register is corrected.** MR-40's "seven read paths" is true of their audit-then-return SHAPE; only `list_analysis_overrides`, `list_audit_log` and `retention_status` raise a `42501`. The other four refuse only on `28000` or `22023`. **Deliberately not audited, and this is the security half:** `28000`, because there is no actor to name and it would let an anonymous caller grow an append-only table with a rejection trigger and no delete path — one row per request, a write-amplification vector; and `22023`, a malformed call from our own console rather than somebody finding out what they can reach. **`audit_log.refused`** marks them, and `list_audit_log` EXCLUDES them unless `p_include_refused` — the console panel's heading is *"Successful reads"* and a refusal arriving there unasked would have turned a true heading false on the day this ran. **Still open, measured not assumed:** a GRANT-level refusal (`permission denied for function list_audit_log`) happens before the body runs, so no in-function mechanism will ever see it. `BE-W102` is closed for in-body refusals only. **Mutants, two-sided:** a helper that records and then SERVES kills 2 tests; one that refuses and records nothing — the original defect — kills 3. The suite separates *refuses* from *records*. **Two defects of mine the existing suite caught, both recorded in the migration:** the drop-and-recreate of `list_audit_log` left it **EXECUTE to PUBLIC**, so `anon` could read the audit trail — caught by `privilege-posture` and `rls`, now revoked and asserted; and I measured the HTTP status **as anon** (401) and generalised it to an identified caller, where PostgREST returns **403** — caught by `analysis-overrides-http`. Also `using hint = null` is itself an error in PL/pgSQL, which one of the three callers triggered. Suite **63 files / 830 tests**, twice; rollbacks clean; drift 75 of 75 |
