...
| **BE-W89** | **`sync_pull` carries no beat-plan entries — and that is only the first of two blockers on the same screen** | `sync_pull`, `BeatPlanRecord`, `app/beat-plan.tsx`, the "On plan" chip in `app/(tabs)/doctors.tsx` | MR-14 B9 / MR-16 B4 | **re-size before estimating** | ? | Measured as a seeded MR: `sync_pull` emits exactly `visit`, `doctor`, `beat_plan` and `clinic_address`, and has **no `beat_plan_entry` entity** — read from the live function definition, not the migration — so `BeatPlanRecordSchema` is `BeatPlanSchema.omit({ entries: true })` while `public.beat_plan_entries` genuinely holds rows. `buildDayRoute` maps `plan.entries` to stops, so the screen would render an empty route. **MR-14 left it on the mock and REMOVED the "On plan" chip rather than ship either as an empty result presented as fact.** <br><br>**MR-16 B4 found the second blocker, and it was found BEFORE the backend work rather than by removing the first one.** `beat_plans.status` is `'submitted'` in **every** fixture — `seed-day.mjs:350` and `fixtures.ts:328` — across 33 rows and all 35 api suites, while `app/beat-plan.tsx:41` filters `.filter((plan) => plan.status === 'approved')`. **So the screen would render an empty route even with the entity shipped.** The enum carries `draft, submitted, approved, rejected` and nothing in this repository ever writes `approved`. <br><br>**Nothing approves a beat plan.** Approvals are MR scope §4.6, in a manager console that is not built. So this task is not "add a missing entity": the real chain is **beat-plan screen → approved plans to render → an approval action that produces them → the manager console that hosts it**. Size it against that. <br><br>Verification, in order: (1) `sync_pull` emits `beat_plan_entry` and `BeatPlanRecord` carries `entries`; (2) a fixture and `seed:day` produce an `approved` plan, with a test asserting the status dimension is no longer single-valued — MR-16's method, since a fixture that varies only `submitted` cannot falsify `status = 'approved'`; (3) the beat-plan screen renders stops that came from Postgres; (4) the "On plan" chip returns, with a test asserting it filters rather than merely appearing |
...
| ID | Title | Changes | Deps | Blocker | Est | Verification |
|---|---|---|---|---|---|---|
| **FE-W38** | **THE APP'S OFFLINE STORY STOPS AFTER THE FIRST WRITE. Only 2 of the 5 writes can be performed with no signal, and one of those by accident.** Measured on the emulator with the Supabase tunnel removed (wifi alone does not cut the app off — `adb reverse` runs over the adb transport). **check-in** queues correctly. **call report** works *only because it still READS the mock at `:4010`*; convert that screen and it joins the others. **consent** and **samples** both render *"Could not load this visit. The app could not reach the server."* — each fetches the visit, and consent also the notices, before it can ask anything. **check-out is unreachable entirely**: `stageOf()` returns `'during'` only for `visit.status === 'in_progress'`, which only `record_check_in` writes and only SERVER-side, so offline the visit screen never leaves the `'before'` stage and offers nothing but check-in. <br><br>**This is the gate, not a rough edge.** FE-G2 is *"8h offline, ≥20 queued writes, then sync"*. Twenty queued writes cannot be produced: an MR with no signal can check in, and then the app has nothing further to offer them for the rest of the visit. <br><br>**Why it is registered and not fixed.** Every one of these screens is reading server-confirmed state before acting, which is this product's founding rule — *"the app does not present a state the server has not confirmed"* — working exactly as designed and producing an app that cannot work offline. The resolution is a product decision with real consequences either way: an optimistic local `in_progress` makes check-out reachable and makes the screen assert something the server has not agreed to; a locally-cached notice makes consent capturable offline and reintroduces the FIX-12 risk of capturing against a version the server no longer considers active. Engineering should not pick between those alone | `stageOf`, `app/visit/[id].tsx`, `app/consent/[visitId].tsx`, `app/samples/[visitId].tsx`, the pulled store | MR-25 D1 | **needs a product decision, then FE-G2** | 8 | The gate itself: 8 hours offline, ≥20 queued writes spanning all five entities, restart, reconnect, each arriving exactly once as the correct entity type. Today's partial run is the baseline — 2 writes, exactly once, `occurred_at` preserved to the millisecond across a 17-minute queue |
