...
| **BE-W89** | **RE-SIZED AND HALF DONE — MR-44.** The SERVER half is complete: `sync_pull` emits `beat_plan_entry`, the contract and the client store carry it, tombstones record the parent plan's scope. **The re-size the original row missed: `beat_plan_entries` had no `updated_at`**, and the pull orders and pages on `(updated_at, id)` — so this was never "add a union-all arm". **The SCREEN half is not started**: `beat-plan.tsx` and `doctors.tsx` still read `createClientForScenario()`, nothing outside `src/sync/` reads `store.beat_plan_entry`, and `BeatPlanRecordSchema` still omits `entries` | `sync_pull`, `packages/core`, `apps/field/src/sync`, and still to come `app/beat-plan.tsx` | MR-14 B9 / MR-16 B4 / MR-44 B | **Option (b): render the plan with its status shown honestly.** Option (a) — marking plans approved so the filter passes — is REFUSED: a plan marked approved that no manager approved is a false record. **And (b) is sound, checked rather than assumed: only ONE consumer reads `status === 'approved'`, the screen's own filter** | **~2 half-days remain** (screen conversion, status wording, zone-correct dates) | Server half: `sync-pull-beat-plan-entries.spec.ts` — the arm reaches the plan's MR and nobody else, ordering is the transport order, the tombstone records the plan's `mr_id`, the mapping still refuses an unmapped table. Screen half, still to do: the plan renders with **"Submitted — not yet approved"**, and the approved wording CANNOT appear for a submitted plan |
