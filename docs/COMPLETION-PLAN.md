...
| **BE-W89** | **SERVER AND CLIENT HALVES DONE — MR-44 and MR-45. One small item remains: the "On plan" chip.** `sync_pull` carries `beat_plan_entry`; `app/beat-plan.tsx` reads the pulled store and renders today's plan with its status stated — *"Submitted — not yet approved"* — never marked approved. Established by ELIMINATION on a Pixel 10 with the mock dead | `sync_pull`, `packages/core`, `apps/field/src/sync`, `src/today/beat-plan-view.ts`, `app/beat-plan.tsx` | MR-14 B9 / MR-16 B4 / MR-44 B / MR-45 B | Option (b), recorded. Option (a) REFUSED: a plan marked approved that no manager approved is a false record | **~0.5 half-day remains** (the chip, reusing `todaysPlan`) | `beat-plan-view.test.ts` (22), `beat-plan-route.test.tsx` (8), `BeatPlanScreen.test.tsx` (+4): the approved wording is asserted ABSENT for every non-approved status with a positive control; today's plan is chosen by the server's territory date at 18:45Z; a past visit does not count as done today; syncing, no-stops and no-plan are distinct. Two mutants each kill exactly one test |
