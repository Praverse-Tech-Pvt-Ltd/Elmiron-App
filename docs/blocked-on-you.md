...
| 5.13 | **The organisation's REGISTERED legal name, as it should appear on a consent notice** — and confirmation that MRs' display names may be shown to doctors | Client | — | **BE-W93 / MR-24 A2.** The consent screen currently tells a doctor *"your rep's employer is the Data Fiduciary for this recording"*. That sentence is the DPDP notice's identification of who holds their data, and it **names nobody** — not the company, not the person standing in front of them. It is not a copy problem: the name is not in the system at all (no name on the JWT, no organisation name on `user_profiles`, no organisations endpoint). **Do not let engineering pick a name to clear this.** The last placeholder here read *"Your rep's company"* and looked on a real screen exactly like what it was. **The cost of waiting is permanent:** `consent_records` is append-only, so every consent captured before the name exists keeps the anonymous notice as its evidence forever — this is the one open item that gets WORSE the longer it stays open rather than merely staying open |
...
| 5.15 | **Offline, may the app show a state the server has not confirmed?** | Client / Operator | — | **FE-W38 / MR-25 D1.** With no signal an MR can check in and then **nothing else** — consent and samples cannot load the visit, and check-out is not even offered, because the screen decides what to show from `visit.status`, which only the SERVER sets. So FE-G2 (8h offline, 20+ queued writes) cannot be reached: those writes cannot be made. **Both answers cost something.** Showing an optimistic local `in_progress` makes the rest of the visit workable offline and breaks the rule that the app never presents what the server has not confirmed. Caching the consent notice makes consent capturable offline and reintroduces FIX-12 — capturing against a version the server no longer holds active. This is the single decision standing between the app and a working offline day |
